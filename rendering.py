...
class Renderer:
    ...
    def draw_grass(self, grid, screen):
        for y in range(len(grid[0])):
            for x in range(len(grid[0][0])):
                pygame.draw.rect(screen, (0, grid[layer_dict['grass']][y][x] * 180 + 75, 0), (self.scale * x, self.scale * y, self.scale, self.scale))
        ...
        for y in range(len(grid[0])):
            for x in range(len(grid[0][0])):
                if grid[layer_dict['water']][y][x] != 0:
                    pygame.draw.rect(screen, (0, 0, grid[layer_dict['water']][y][x] * 50 + 50),
                                     (50 * x, 50 * y, 50, 50))
    ...
    def draw_temp(grid, screen):
        for y in range(len(grid[0])):
            for x in range(len(grid[0][0])):
                pygame.draw.rect(screen, (grid[layer_dict['temperature']][y][x] * 100 + 50, 0, 0),
                                 (50 * x, 50 * y, 50, 50))
    ...
    def draw_height(grid, screen):
        for y in range(len(grid[0])):
            for x in range(len(grid[0][0])):
                pygame.draw.rect(screen, (grid[layer_dict['heigh']][y][x] * 200, grid[layer_dict['heigh']][y][x] * 200,
    ...
    def draw_creature(self, c, screen):
        (N, W, S, E) = [0, 1, 2, 3]
        rect = pygame.Rect(c.x * self.scale, c.y * self.scale, self.scale, self.scale)
        pygame.draw.rect(screen, black, rect)
        if c.d == S:
            pygame.draw.line(screen, red, ((c.x * self.scale) + self.scale / 2, (c.y * self.scale) + self.scale / 2),
